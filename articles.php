...
if(!$_SESSION){
  ...
  $every_page_article_num=$_SESSION["every_page_article_num"];
}
$sql="SELECT * FROM reading ORDER BY id DESC";
$article_num=getResultNum($sql);
$total_page_num=((int)($article_num/$every_page_article_num))+1;
?>
        ...
        <div class="col-md-10 col-md-offset-1 col-lg-10 col-lg-offset-1">    
          <table class='table table-bordered table-hover table-responsive' width='100%' style="margin-bottom: 0 !important;">
            ...
            if($current_page==$total_page_num){
              $rest_article_num=$article_num-($total_page_num-1)*$every_page_article_num;
              showArticleTable($rest_article_num, $current_page, $every_page_article_num, $rows);
            }else{
              showArticleTable($every_page_article_num, $current_page, $every_page_article_num, $rows);
            }
            ?>
          </table>
              ...
              <ul class="pagination">
                <li>
                  <a aria-label="Previous">
                    <form action="getArticleList.php" method="POST">
                ...
                if($total_page_num<5){
                  for($i=0; $i<$total_page_num; $i++){
                    showArtPagination($i+1, $current_page);
                  }
                  ...
                  for($i=0; $i<5; $i++){
                    showArtPagination($current_page-2+$i, $current_page);
                  }
                }else if($current_page>=4 && $total_page_num-$current_page==1){
                  for($i=0; $i<5; $i++){
                    showArtPagination($current_page-3+$i, $current_page);
                  }
                }else if($current_page>=4 && $total_page_num-$current_page==0){
                  for($i=0; $i<5; $i++){
                    showArtPagination($current_page-4+$i, $current_page);
                  }
                }
                ?>
                <li>
...
<script type="text/javascript">
  function confirm_del(id){
    console.log(id);
    var res=confirm("您确定删除该文章？");
    if(res==true){
      var path="deleteArticle.php?id="+id;
